...
vec2d_t bilinear_c(float X, float Y, vec2d_t v)
{
	int X_0 = int(X);
	int Y_0 = int(Y);
	int X_1 = int(X + 1);
	...
	v_0_0.x = data_c[X_0 + WIDTH * Y_0].x;
	v_0_0.y = data_c[X_0 + WIDTH * Y_0].y;
	v_1_0.x = data_c[X_1 + WIDTH * Y_0].x;
	v_1_0.y = data_c[X_1 + WIDTH * Y_0].y;
	v_0_1.x = data_c[X_0 + WIDTH * Y_1].x;
	v_0_1.y = data_c[X_0 + WIDTH * Y_1].y;
	v_1_1.x = data_c[X_1 + WIDTH * Y_1].x;
	v_1_1.y = data_c[X_1 + WIDTH * Y_1].y;
...
double noise[WIDTH * SCALE_X][HEIGHT * SCALE_Y]; //the noise array
void generateNoise()
{
...
double LIC(int L, list<pnt2d_t> pnts)
{
	...
	double a = s_0 - L * 1.0 / 2;
	double b = s_0 + L * 1.0 / 2;
...
pnt2d_t RKA(float x, float y)
{
	float deltaT = 1.0;
	vec2d_t v_i;
	v_i = bilinear((x * 1.0) / (SCALE_X * 1.0), (y * 1.0) / (SCALE_Y * 1.0), v_i);
	pnt2d_t p;
	...
	p.x = (float)x + (1.0 / 6.0) * deltaT * (v_i.x + 2 * v_1_i.x + 2 * v_2_i.x +v_3_i.x);
	p.y = (float)y + (1.0 / 6.0) * deltaT * (v_i.y + 2 * v_1_i.y + 2 * v_2_i.y +v_3_i.y);

	return p;
}

pnt2d_t RKA_c(float x, float y)
{
	float deltaT = 1.0;
	vec2d_t v_i;
	v_i = bilinear_c((x * 1.0) / (SCALE_X * 1.0), (y * 1.0) / (SCALE_Y * 1.0), v_i);
	pnt2d_t p;
...
int main()
{
	...
	inf.read(reinterpret_cast<char*>(data), WIDTH * HEIGHT * sizeof(vec2d_t));
...
//	cout << pnt3.x << " " << pnt3.y << endl;
	...
	list<pnt2d_t> pnts;
			for (int i = 0; i < LENGTH / 2; i++) {
				...
				if (0 <= pnt1.x && pnt1.x <= width && 0 <= pnt1.y && pnt1.y <= height) {
					pnts.push_back(pnt1);
					tmpLen++;
				}
				if (0 <= pnt3.x && pnt3.x <= width && 0 <= pnt3.y && pnt3.y <= height) {
...
*/	
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
			...
			image[index].a = 1;
			*/
			image[index].r = noise[x][y];
			image[index].g = noise[x][y];
			image[index].b = noise[x][y];
			image[index].a = 1;
